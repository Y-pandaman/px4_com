/*
 * @Author: 姚潘涛
 * @Date: 2024-07-09 19:50:01
 * @LastEditors: 姚潘涛
 * @LastEditTime: 2024-07-10 14:34:43
 * @Description:
 *
...
Eigen::Quaterniond
transform_orientation_aircraft_to_baselink(const Eigen::Quaterniond& q) {
    // Static quaternion needed for rotating between aircraft and base_link
    // frames +PI rotation around X (Forward) axis transforms from Forward,
    // Right, Down (aircraft) Fto Forward, Left, Up (base_link) frames.
...
Eigen::Quaterniond
transform_orientation_baselink_to_aircraft(const Eigen::Quaterniond& q) {
    // AIRCRAFT_BASELINK_Q is the Static quaternion needed for rotating between
    // aircraft and base_link frames +PI rotation around X (Forward) axis
    // transforms from Forward, Right, Down (aircraft) Fto Forward, Left, Up
    // (base_link) frames.
    Eigen::Vector3d aaa(M_PI, 0.0, 0.0);
    Eigen::Quaterniond AIRCRAFT_BASELINK_Q = quaternion_from_rpy(aaa);
    return q * AIRCRAFT_BASELINK_Q;
...
Eigen::Quaterniond
transform_orientation_ned_to_enu(const Eigen::Quaterniond& q) {
    // NED_ENU_Q is the Static quaternion needed for rotating between ENU and
    // NED frames NED to ENU: +PI/2 rotation about Z (Down) followed by a +PI
    // rotation around X (old North/new East) ENU to NED: +PI/2 rotation about Z
    // (Up) followed by a +PI rotation about X (old East/new North)
...
Eigen::Quaterniond
transform_orientation_enu_to_ned(const Eigen::Quaterniond& q) {
    // NED_ENU_Q is the Static quaternion needed for rotating between ENU and
    // NED frames NED to ENU: +PI/2 rotation about Z (Down) followed by a +PI
    // rotation around X (old North/new East) ENU to NED: +PI/2 rotation about Z
    // (Up) followed by a +PI rotation about X (old East/new North)
