...
float abs_distance1 =
    10;  //为模式4中穿越2门与识别图像之间的过度而设置的最小距离值
//--------------------------------------------输出--------------------------------------------------
...
void lidar_cb(const sensor_msgs::LaserScan::ConstPtr &scan) {
  ...
  for (int i = 0; i < count; i++) {
    if (i + 180 > 359)
      ...
      Laser.ranges[i] = Laser_tmp.ranges[i + 180];
    // cout<<"tmp: "<<i<<" l:"<<Laser_tmp.ranges[i]<<"|| Laser:
    // "<<Laser.ranges[i]<<endl;
  }
  // cout<<"//////////////"<<endl;
  //计算前后左右四向最小距离
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber lidar_sub =
      nh.subscribe<sensor_msgs::LaserScan>("/scan", 1000, lidar_cb);  // dyx
  // ros::Subscriber lidar_sub =
  // nh.subscribe<sensor_msgs::LaserScan>("/lidar2Dscan", 1000, lidar_cb);//dyx

  //【订阅】darknet数据
  ros::Subscriber darknet_found_sub = nh.subscribe<std_msgs::Int8>(
      "/darknet_ros/found_object", 1, darknet_found_cb);  // dyx
  ros::Subscriber darknet_box_sub =
      nh.subscribe<darknet_ros_msgs::BoundingBoxes>(
          "/darknet_ros/bounding_boxes", 1, darknet_box_cb);  // dyx

  //【订阅】无人机当前位置 坐标系 NED系
  // ros::Subscriber position_sub =
  // nh.subscribe<geometry_msgs::Pose>("/drone/pos", 100, pos_cb);
  ros::Subscriber position_sub = nh.subscribe<geometry_msgs::PoseStamped>(
      "/mavros/local_position/pose", 100, pos_cb);  // dyx

  // 【发布】发送给position_control.cpp的命令
  ros::Publisher command_pub =
      nh.advertise<px4_command::command>("/px4/command", 10);
  ...
  cout
      << "Please check the parameter and setting，1 for go on， else for quit: "
      << endl;
  cin >> check_flag;
  if (check_flag != 1) {
    return -1;
  ...
  while (ros::ok()) {
    ...
    else if(reach_door_flag[0]&&!reach_door_flag[1])
    {
       if(detect_num&&abs_distance1<0.3)
       {
          ...
          abs_distance1 = sqrt((pos_drone.pose.position.x - door_center_x[1]) *
    (pos_drone.pose.position.x - door_center_x[1]) + (pos_drone.pose.position.y
    - door_center_y[1]) * (pos_drone.pose.position.y - door_center_y[1]));
    }
    ...
    */
    if (!reach_door_flag[0])  // finddoorcentor(0);
    {
      ...
      float abs_distance;
      abs_distance = sqrt((pos_drone.pose.position.x - door_x - 0.1) *
                              (pos_drone.pose.position.x - door_x - 0.1) +
                          (pos_drone.pose.position.y - door_y) *
                              (pos_drone.pose.position.y - door_y));
      if (abs_distance < 0.3) {
      ...
      float abs_distance;
      abs_distance = sqrt((pos_drone.pose.position.x - final_x) *
                              (pos_drone.pose.position.x - final_x) +
                          (pos_drone.pose.position.y - final_y) *
                              (pos_drone.pose.position.y - final_y));
      if (abs_distance < 0.3) {
        flag_land = 1;
      }
...
void collision_avoidance(float target_x, float target_y) {
  ...
  //比如，y方向接近0，但x还差很多，但x方向有障碍，这个时候按discx
  //cy的大小，缓解y的难题。
...
void finddoorcentor(int i) {
  ...
  if (!door_center_x[i]) {
    ...
    for (int k = theta1; k > 0; k--) {
      ...
      // cout<<"k: "<<k<<" l: "<<l<<" Laser: "<<Laser.ranges[k]<<" dl:
      // "<<dl<<endl;
    }
    for (int k = 0; k <= theta2; k++) {
      ...
      // cout<<"k: "<<359-k<<" l: "<<l<<" Laser: "<<Laser.ranges[359-k]<<" dl:
      // "<<dl<<endl;
    }
    cout << "door angle num: " << door_angle.size() << endl;
    cout << "first :" << door_angle.front() << "last one: " << door_angle.back()
         << endl;
    ...
    cout << "door position: " << door_center_x[i] << " " << door_center_y[i]
         << endl;
  }
  collision_avoidance(door_center_x[i] + 0.1, door_center_y[i]);
  float abs_distance;
  abs_distance = sqrt((pos_drone.pose.position.x - door_center_x[i] - 0.1) *
                          (pos_drone.pose.position.x - door_center_x[i] - 0.1) +
                      (pos_drone.pose.position.y - door_center_y[i]) *
                          (pos_drone.pose.position.y - door_center_y[i]));
...
void detect_nav() {
  ...
  abs_distance = sqrt((pos_drone.pose.position.x - pic_target[0] + 0.8) *
                          (pos_drone.pose.position.x - pic_target[0] + 0.8) +
                      (pos_drone.pose.position.y - pic_target[1]) *
                          (pos_drone.pose.position.y - pic_target[1]));
